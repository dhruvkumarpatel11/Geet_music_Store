...
if($query->num_rows > 0){
    ...
    if($delete){
        ...
        echo '<script>alert("Audio Data successfully deleted."); location.replace("./custom.php")</script>';
    }else{
        echo '<script>alert("Unable to Delete the audio data."); location.replace("./custom.php")</script>';
    }
    
}else{
    echo '<script>alert("Unknown Audio Data ID."); location.replace("./custom.php")</script>';
}